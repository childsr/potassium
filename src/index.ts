...
export interface AppBuilder<State,Context,TriggerMap extends Record<string,any>> extends AppShared<State,Context,TriggerMap> {
  onStart(handler: VoidHandler<AppRuntime<State,Context,TriggerMap>>): void
  start(initialState: State): Promise<State>
}
export interface AppRuntime<State,Context,TriggerMap extends Record<string,any>> extends AppShared<State,Context,TriggerMap> {
...
class Builder<State,Context,TriggerMap extends Record<string,any>> implements AppBuilder<State,Context,TriggerMap> {
  ...
  private bindTriggerHandler<K extends keyof TriggerMap>(triggerId: K, handler: Handler<AppRuntime<State,Context,TriggerMap>, TriggerMap[K]>): symbol {
    const id = Symbol()
  ...
  onStop(handler: VoidHandler<AppRuntime<State,Context,TriggerMap>>): symbol {
    const id = Symbol()
...
type HandlerMap<State,Context,TriggerMap extends Record<string,any>> = {
  triggerHandlers: Map<keyof TriggerMap, Map<symbol, Handler<AppRuntime<State,Context,TriggerMap>,any>>>
  onStartHandlers: Set<VoidHandler<AppRuntime<State,Context,TriggerMap>>>
  onStopHandlers: Map<symbol, VoidHandler<AppRuntime<State,Context,TriggerMap>>>
  eventHandlers: Map<symbol, StreamHandlerPair<AppRuntime<State,Context,TriggerMap>,any>>
}
type RuntimeParameters<State,Context,TriggerMap extends Record<string,any>> = {
  initialState: State
  context: Context
...
class Runtime<State,Context,TriggerMap extends Record<string,any>> implements AppRuntime<State,Context,TriggerMap> {
  ...
  private triggerHandlers: Map<keyof TriggerMap, Map<symbol, Handler<AppRuntime<State,Context,TriggerMap>,any>>>
  private onStopHandlers: Map<symbol, VoidHandler<AppRuntime<State,Context,TriggerMap>>>

  private cancelControllers: Map<symbol, CancelController> = new Map()

  private resolve: (finalState: State) => void

  private bindEventStream<T>(eventStream: EventStream<T>, handler: Handler<AppRuntime<State,Context,TriggerMap>,T>, id: symbol = Symbol()): symbol {
  ...
  constructor(parameters: RuntimeParameters<State,Context,TriggerMap>) {
